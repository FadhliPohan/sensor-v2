...
function get_ket($suhu,$nilai){
	...
	$set="SELECT * FROM `tb_rule_segitiga` WHERE is_golongan='$suhu' and nilai='".$nilai."' GROUP BY is_sub_golongan,keterangan_nilai";
					...
					<table class="table table-bordered" style="width:300px">
						...
						<!-- <table>
							<tr> -->
								...
								if(!empty($h)){
									echo '<span style="margin-left: 380px;">Hot</span>';
								}
								if(!empty($vh)){
				...
				<table class="table table-bordered" style="width:900px">
					...
					while($row=mysql_fetch_array($sql)){
						...
						";
							...
							if(nilai_parameter_a($row['is_sub_golongan'],$row['keterangan'],$_POST['suhu'],$row['keterangan_nilai'],0,"h") == ""){
...
<div style="height:30px;width:800px;background-color:white;z-index: 999999999999999999999999999999999999999999999999999999999999;margin-top:30px; position:absolute ">
						<!-- <table>
							<tr> -->
								...
								if(!empty($vc2)){
									echo '<span style="margin-left: 160px;">Very Dry</span>';
								}
								if(!empty($c2)){
									echo '<span style="margin-left: 170px;">Dry</span>';						
								}
								if(!empty($n2)){
			...
			<div id="chartContainer2" style="height: 370px;width:800px;"></div>
				...
				<table class="table table-bordered" style="width:900px">
					...
					while($row=mysql_fetch_array($sql)){
						...
						";
			if(nilai_parameter_a($row['is_sub_golongan'],$row['keterangan'],$_POST['sm'],$row['keterangan_nilai'],1,"h") == ""){
...
<!-- PERHITUNGAN UNTUK HUMIDITY -------------------------------------------------------------------------------------------->
	...
	<td>$status_d=".nilai_parameter_a($row['is_sub_golongan'],$row['keterangan'],$_POST['hm'],$row['keterangan_nilai'],2,"b")."</td>
						...
						";
							if(nilai_parameter_a($row['is_sub_golongan'],$row['keterangan'],$_POST['hm'],$row['keterangan_nilai'],2,"h") == ""){
...
<!-- PERHITUNGAN UNTUK LIGHT -------------------------------------------------------------------------------------------->
				...
				<table class="table table-bordered" style="width:900px">
					...
					while($row=mysql_fetch_array($sql)){
						...
						";
							if(nilai_parameter_a($row['is_sub_golongan'],$row['keterangan'],$_POST['li'],$row['keterangan_nilai'],3,"h") == ""){
...
<!-- PERHITUNGAN UNTUK FUZYFICATION -------------------------------------------------------------------------------------------->
					...
					<table class='table table-bordered' style="width:930px">
						...
						function get_rule_kondisi($kondisi_1,$kondisi_2,$kondisi_3){
							$sql=mysql_query("SELECT * from tb_rule_segitiga_kondisi where kondisi_1 LIKE '%$kondisi_1%' and kondisi_2 LIKE '".$kondisi_2."'  and kondisi_3 LIKE '".$kondisi_3."' ");
								while($row=mysql_fetch_array($sql)){
									$kondisi=$row['hasil'];
								}
								return $kondisi;
...
<!-- PERHITUNGAN UNTUK FUZYFICATION -------------------------------------------------------------------------------------------->
					...
					<table class='table table-bordered' style="width:930px">
						...
						function get_duration($kondisi_1,$nilai_set){
							$nilai=0;
							$sql=mysql_query("SELECT * from tb_rule_segitiga_duration where keterangan='".$kondisi_1."' LIMIT 0,4");
								while($row=mysql_fetch_array($sql)){
...
<!-- PERHITUNGAN UNTUK DEFUZYFICATION -------------------------------------------------------------------------------------------->
					...
					<?php
						...
						//	echo"<tr><td>".round($hasil_meni)." Detik</td></tr>";
							mysql_query("INSERT INTO tb_hasil_menit(tanggal,suhu,sm,humidity,li,defuz,nilai)VALUES('".$_POST['tanggal']."','".$_POST['suhu']."','".$_POST['sm']."','".$_POST['hm']."','".$_POST['li']."','$gol','".$hasil_meni."')");	
					?>
					  </div>
				 </div>
				</div>
...
type: "line", //change it to line, area, column, pie, etc
		dataPoints: [
			...
			{ x: 9000, y: 0},
			{ x: <?php echo $getagak_terang['nilai'] ?>, y: <?php echo $getagak_terang['hasil'] ?> },	
			{ x: 19500, y: 1 },
			{ x: 30000, y: 0 },
			<?php }else{ ?>
